...
async def get_trigger_history(
    zone_code: str = None, limit: int = 20,
    current_user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
...
async def simulate_trigger(
    zone_code: str, trigger_type: str,
    current_user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Simulate a trigger event for demo purposes."""
    values = {
        "HEAVY_RAIN": 95.5, "FLOOD": 1.0, "HEAT": 44.5,
        "AQI": 435, "ORDER_SUSPENSION": 3.5,
    }
    ...
    trigger = await TriggerMonitor.fire_trigger(
        db, zone_code=zone_code, trigger_type=trigger_type,
        value=value, sources_agreeing=3,
    )

    return {
        "message": f"Trigger {trigger_type} fired for zone {zone_code}",
        "trigger_id": trigger.id, "severity": trigger.severity,
