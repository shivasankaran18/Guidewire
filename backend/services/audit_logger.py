...
class AuditLogger:
    ...
    @staticmethod
    def _compute_hash(entity_type, entity_id, action, actor_id, timestamp, previous_hash="", data=None):
        payload = json.dumps({
    ...
    async def log(
        ...
        ip_address: str = None, device_fingerprint: str = None, metadata: dict = None,
    ) -> AuditLog:
        ...
        entry_hash = AuditLogger._compute_hash(
            entity_type=entity_type, entity_id=entity_id,
            action=action, actor_id=actor_id or "SYSTEM",
            timestamp=now.isoformat(), previous_hash=previous_hash, data=new_state,
        )
        ...
        audit_entry = AuditLog(
            ...
            metadata_=metadata, created_at=now,
        )
        db.add(audit_entry)
        await db.flush()
        return audit_entry
        ...
        for i, entry in enumerate(entries):
            if i == 0:
                if entry.previous_hash != "GENESIS":
                    broken_links.append({"entry_id": entry.id, "expected_previous": "GENESIS", "actual_previous": entry.previous_hash})
            else:
                if entry.previous_hash != entries[i - 1].entry_hash:
                    broken_links.append({"entry_id": entry.id, "expected_previous": entries[i - 1].entry_hash, "actual_previous": entry.previous_hash})

        return {"valid": len(broken_links) == 0, "entries_checked": len(entries), "broken_links": broken_links}
